...
#install.packages("future.apply", type = "source")
...
# Compute BUB entropy 
############################
setwd("/Users/olivier_2/Documents/En_cours/Labo/Manipes/OG3700")
mat <- read.csv('gene_expression_by_time.csv', row.names = 1)
time <- (mat[30035,])
times <- unique(as.character(time))

M1 <- mat[,mat[30035,]==times[1]]
M1 <- t(M1[-30035,])
M2 <- mat[,mat[30035,]==times[2]]
M2 <- t(M2[-30035,])
M3 <- mat[,mat[30035,]==times[3]]
M3 <- t(M3[-30035,])
M4 <- mat[,mat[30035,]==times[4]]
M4 <- t(M4[-30035,])
M5 <- mat[,mat[30035,]==times[5]]
M5 <- t(M5[-30035,])

# ================
# Compute entropy
# ================
H_M1 <- future_sapply(1:ncol(M1), function(i) estimateur_bub_entropie_discrete(as.numeric(M1[, i])))
H_M2 <- future_sapply(1:ncol(M2), function(i) estimateur_bub_entropie_discrete(as.numeric(M2[, i])))
H_M3 <- future_sapply(1:ncol(M3), function(i) estimateur_bub_entropie_discrete(as.numeric(M3[, i])))
H_M4 <- future_sapply(1:ncol(M4), function(i) estimateur_bub_entropie_discrete(as.numeric(M4[, i])))
H_M5 <- future_sapply(1:ncol(M5), function(i) estimateur_bub_entropie_discrete(as.numeric(M5[, i])))

# ================
# Save as a dataframe
# ================
gene_columns <- colnames(M1)
entropy_matrix <- data.frame(
  t1 = H_M1,
  t2 = H_M2,
  t3 = H_M3,
  t4 = H_M4,
  t5 = H_M5,
  row.names = gene_columns,
  check.names = FALSE 
)

write.csv(entropy_matrix, file = "gene_entropy_by_time.csv", row.names = TRUE, quote = FALSE)
